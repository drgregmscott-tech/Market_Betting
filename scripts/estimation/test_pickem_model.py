...
def test_prizepicks_demon_and_goblin_are_not_scored(odds_type):
    """Session 2.55: their payout is set per leg (13.5x vs 5.25x for a Demon
    on two different bets), so no constant can price them."""
    weekly_fixture = build_nfl_weekly_fixture()
    props = pd.DataFrame([dict(
        platform="prizepicks", source_line_id="1", player_name="Player One",
        sport="nfl", stat_type="Pass Yards", line=265.5, odds_type=odds_type,
    )])
    original_fetch = NFL_PLUGIN.fetch_stats
    NFL_PLUGIN.fetch_stats = lambda season: weekly_fixture
    try:
